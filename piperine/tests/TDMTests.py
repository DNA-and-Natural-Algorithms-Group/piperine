...
class TestTDM(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.seqs_file = pkg_resources.resource_filename('piperine', 'tests/test_data/test_tdm.seqs')
    ...
    def test_NotToInteract(self):
        ...
        def nti_to_sequence(nti, sd):
            outdict = {}
            for key in nti.keys():
                outdict[sd[key]] = to_sequence(nti[key], sd)
            return outdict
        true_nti_seqs = nti_to_sequence(true_nti, self.seq_dict)
...
def suite():
    tests = ['test_TopStrandlist', 'test_BaseStrandlist', 'test_TopStranddict', 'test_TopStranddict']
